...
def companion_pa(ra_diff,dec_diff,pa2):
    pa = np.degrees(np.arctan2(ra_diff, dec_diff))
    mod_360 = (((pa2-pa) + 180 + 360) % 360) - 180
    pa = pa + (2003.625 - 2016)/(2003.625-2016) * mod_360
    return pa
...
if __name__=="__main__":
    ...
    companions_ra_dec = [(1292,4.062898388828774,-79.8503334744852),
                         ...
                         (22715,73.26963747632014,22.23384233042775),
    ...
    for companion in companions_ra_dec:
        ...
        dec_diff = companion[2] - star['gaia_dec']
        ra_diff = (companion[1] - star['gaia_ra'])*np.cos(np.radians(star['gaia_dec']))
        pa2 =[angle[1] for angle in position_angles if angle[0]==companion[0]]
        pa1 = companion_pa(ra_diff,dec_diff,pa2[0].value)
        pa_comparison.append((companion[0],pa1%360,pa2[0].value%360))
