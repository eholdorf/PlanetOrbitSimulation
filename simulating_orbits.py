...
def chisq(mu_S1,mu_S2,mu_E1,mu_E2):
    ...
    ----------
    mu_S1:
        proper motion for first epoch
    mu_S2:
        proper motion for second epoch
    """
...
def calc_chi_sq(sim,data_row):
    ...
    sim_pm_gaia = proper_motion(sim[0,1], sim[0,2], 2015.0, sim[1,1], sim[1,2], 2017.0)
    
    # simulation values of RA and DEC between hipparcos and gaia
    sim_pm_hg = proper_motion(sim[0,0], sim[0,2], 1991.25, sim[1,0], sim[1,2], 2017.0)

    chi2 = 0
    chi2 += (sim_pm_gaia[0]-sim_pm_hg[0])**2/(data_row['pmra_hg_error']**2 + data_row['pmra_gaia_error']**2)
    chi2 += (sim_pm_gaia[1]-sim_pm_hg[1])**2/(data_row['pmdec_hg_error']**2 + data_row['pmdec_gaia_error']**2)
...
if __name__=="__main__":
    ...
    data_dir = '/Users/mireland/Google Drive/EDR3_Planets/' #For this directory, use './'
    n_sims = 1000
