...
if __name__=="__main__":
    ...
    for this_bprp, parallax in zip(bp_rp[:n_stars], 
        ...
        for i in range(n_sims): 
            ...
            radecs.append(simulate_orbit(2021,T,a,e,Omega,omega,i,t_k,m_star,
                                         m_planet,parallax))
            params = [2021,T,a,e,Omega,omega,i,t_k,m_star,m_planet,parallax]
            sim_params.append(params)
        all_star_radecs.append(radecs)
        all_star_params.append(sim_params)
    ...
    f = open('high_chisq.txt','w')
    for i in range(n_stars):
        for j in range(n_sims):
            if chis[i,j]>9.5:
                f.write('Star Number: '+str(i)+' Simulation Number: '+str(j)+
                        ' Chisq: '+str(chis[i,j])+'\n')
